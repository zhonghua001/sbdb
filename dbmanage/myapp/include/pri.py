...
def set_ins(insname,setip,setport,status,setdbtype,comments,create_user):
    if len(setip)>0:
        insname.ip = setip
    if len(setport)>0:
        insname.port = setport
    insname.status = str.upper(status.encode())
    insname.db_type = setdbtype
    insname.comments = comments

    insname.update_user = create_user
    insname.save()
    return insname

def create_dbinstance(setip,setport,status,setdbtype,comments,create_user):
    if len(setip)>0 and len(setport)>0:
        insname = Db_instance(ip=setip,port=setport,status=status,db_type=setdbtype,create_user=create_user,comments=comments)
        insname.save()
        return insname
