...
def table_check():
    ...
    def threading_check(i):
        try:
            print i.ip, i.port, i.comments
            ...
            sql = "SELECT *  FROM (SELECT \
                      ...
                      EXTRA='auto_increment')a \
                      where a.AUTO_INCREMENT/a.MAX_VALUE>0;"
            ...
            sql = "SELECT TABLE_SCHEMA,TABLE_NAME,\
                      ...
                      ,'" + str(i.id) + "' FROM  INFORMATION_SCHEMA.TABLES "
            param, col = get_data(i, sql)
            # print param
    ...
    # for i in Db_instance.objects.filter(mysql_monitor__table_check=1, db_type='mysql'):

    thread_pool = []
    print (datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    for i in Db_instance.objects.filter(mysql_monitor__table_check=1,db_type='mysql'):
        t = threading.Thread(target=threading_check,args=(i,))
...
def get_data(a,sql):
    ...
    if tar_username <> 0:
        ...
        if col[0] == 'error':
            mailto = map(lambda x: x.user.email,filter(lambda x: len(x.user.email) > 0, User_profile.objects.filter(task_email__gt=0)))
            html_content = loader.render_to_string('include/mail_template.html', locals())
            msg = 'Table check error:' + results[0][0]
            sendmail(msg, mailto, html_content)

        return results,col
    return (['No admin user'],''),['error']
...
def get_dupreport(hosttag,db,email=''):
    ...
    if incept.pttool_switch!=0:
        ...
        try:
            ...
            db_account = Db_account.objects.get(instance=instance,db_account_role='admin')
            tar_dbname = db
        ...
        if os.path.isfile(incept.pttool_path+'/pt-duplicate-key-checker') :
            ...
            if email != '':
                html_content = loader.render_to_string('include/mail_template.html', locals())
                sendmail('DUPKEY CHECK ON '+tar_host+':'+tar_port+'----'+db , mailto, html_content)
            else:
                return dup_result
        else :
