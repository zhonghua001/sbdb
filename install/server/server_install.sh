...
case $yum1 in
	yes|y|Y|YES)
	    if (cat $os | grep "centos");then

            yum install -y epel-release
            yum install -y gcc expect python-pip python-devel ansible smartmontools dmidecode libselinux-python
		elif (cat $os |grep "Ubunto");then
		    apt install -y epel-release
		    apt install -y gcc expect python-pip python-dev ansible smartmontools dmidecode libselinux-python1
		fi
...
# Ubuntu need ln -s /usr/bin/nodejs /usr/bin/node
# 分发代码
